...
Matrix createLocalK(int e,mesh &m){
    ...
    productRealMatrix(-(d*Ae)/(8*(D*D)),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixG);


    //Preparando MatrixH
    l = m.getParameter(L);
    productRealMatrix((l*J)/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixH);
...
Vector createLocalb(int e,mesh &m){
    ...
    float f_x = m.getParameter(F_X);
    float f_y = m.getParameter(F_Y);
